...
class Admin {
	...
	public function __construct() {
		...
		// Manually trigger index job instead of waiting for cron (@since 1.x)
	...
	public function import_templates_form() {
		...
		<div id="bricks-admin-import-wrapper">
			...
			<a id="bricks-admin-template-bundles" href="<?php echo admin_url( 'edit-tags.php?taxonomy=template_bundle' ); ?>" class="page-title-action"><?php esc_html_e( 'Bundles', 'bricks' ); ?></a>
			<a id="bricks-admin-template-tags" href="<?php echo admin_url( 'edit-tags.php?taxonomy=template_tag' ); ?>"class="page-title-action"><?php esc_html_e( 'Tags', 'bricks' ); ?></a>
	...
	public function bricks_template_posts_custom_column( $column, $post_id ) {
		...
		if ( $column === 'template_thumbnail' ) {
			...
			// Get automatically-generated template screenshot from custom directory (@since 1.10)
				...
				if ( $file_path ) {
					$file_url = str_replace( $custom_dir, $custom_url, $file_path );
	...
	public function builder_tab_html() {
		// Return: Post type not supported for Bricks OR user role is enabled for builder
		if ( ! Helpers::is_post_type_supported() || ! Capabilities::current_user_can_use_builder() ) {
			return;
		}
	...
	public function delete_template_screenshots() {
		...
		if ( is_dir( $custom_dir ) ) {
			// Find and delete all files (including hidden, etc.)
			$existing_files = glob( $custom_dir . '{,.}*', GLOB_BRACE );

	...
	public function render() {
		...
		if ( $type === 'posts' && $carousel_query && $carousel_query->count === 0 ) {
			...
			foreach ( $carousel_posts as $item_index => $item ) {
				...
				$aria_label = get_post_meta( $item->ID, '_wp_attachment_image_alt', true );
				$image_url  = false;
				...
				if ( $image_url ) {
					...
					if ( $lightbox ) {
						$lightbox_image_src = wp_get_attachment_image_src( $item->ID, $settings['imageLightboxSize'] ?? 'full' );
						...
						$this->set_attribute( "a-$item_index", 'aria-label', esc_html__( 'Open image in lightbox', 'bricks' ) . ' (ID: ' . $item->ID . ')' );

						// Lightbox caption (@since 1.10)
						$lightbox_caption = isset( $settings['lightboxCaption'] ) && $item->ID ? wp_get_attachment_caption( $item->ID ) : false;

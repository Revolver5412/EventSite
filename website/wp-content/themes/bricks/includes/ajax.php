...
class Ajax {
	...
	public function save_template_screenshot() {
		...
		if ( isset( $_POST['screenshot'] ) && ! empty( $_POST['screenshot'] ) ) {
			...
			if ( strpos( $screenshot_data, 'data:image/webp;base64,' ) === false ) {
				wp_send_json_error( 'Invalid screenshot data' );
			}
			...
			if ( ! file_exists( $custom_dir ) ) {
				$directory_created = wp_mkdir_p( $custom_dir );
			}

			// Find existing screenshots for this post ID and delete them if they exist
			if ( is_dir( $custom_dir ) ) {
				$existing_files = glob( $custom_dir . "template-screenshot-$post_id-*.webp" );
				foreach ( $existing_files as $file ) {
					if ( file_exists( $file ) ) {
						@unlink( $file ); // Delete each existing file
					}
				}
			}
			...
			$timestamp = time();
			$filename  = "template-screenshot-$post_id-$timestamp.webp";
			$filepath  = $custom_dir . $filename;

			// Save the new screenshot
			if ( file_put_contents( $filepath, $screenshot_data ) ) {
			...
			wp_send_json_error( 'No screenshot data provided' );
		}
	}

		...
		if ( $templates_query->have_posts() ) {
			foreach ( $templates_query->get_posts() as $template ) {
				...
				if ( has_post_thumbnail( $template_id ) ) {
					...
					// Pattern to find the most recent screenshot file for the given template ID
					$pattern        = $custom_dir . "template-screenshot-$template_id-*.webp";
					$matching_files = glob( $pattern, GLOB_NOSORT );

					if ( ! empty( $matching_files ) ) {
						// Sort files by name assuming the timestamp is part of the filename
						usort(
							$matching_files,
							function ( $file_a, $file_b ) {
								return strcmp( $file_b, $file_a ); // Reverse order to get the most recent first
							}
						);
